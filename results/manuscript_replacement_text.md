...
### 16-kbit/s Opus Effects vs. VAD Degradation (Table II Reframing)
```latex
...
Under full front-end degradation, this automated index exhibits a statistically significant systematic relative underestimation of $-9.38\%$ (95\% CI: $[-12.35\%, -6.18\%]$) across episodes relative to clean audio. Furthermore, severity underestimation is correlated with ground-truth block density ($r = -0.20, p = 0.0015$; Spearman $\rho = -0.20, p = 0.0023$). For clinical telepractice applications incorporating automated dysfluency tracking, system architects should consider transmitting an un-excised audio channel or bypassing upstream VAD for diagnostic stuttering evaluation, as full front-end degradation reduces the episode-level predicted dysfluency index relative to clean predictions.
```
